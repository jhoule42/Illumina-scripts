...
carte_total_diff = MSD.Open(path + "/Actu_ete/elevation_angle_90-azimuth_angle_0-wavelength_397.0.hdf5")
...
print("\n------------------------------------")
for index, scen in enumerate(nom_scenarios):
    os.chdir(nom_scenarios[index])
    ...
    print("MSD directory created: " + nom_scenarios[index])
    os.chdir("..")
    ...
    for index2, spct in enumerate(liste_spectres):
        ...
        plt.savefig("/home/jhoule42/Documents/Resultats_Sherb_R2/Carte_contrib/%s_%s.png" % (nom_scenarios[index], nom_spectres[index2]), bbox_inches="tight")
        plt.close()

    os.chdir("Results_Sherb_R2")
